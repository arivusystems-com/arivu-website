...
import { CheckItem, SectionHeading } from '@/components/site/section'
import { apps, demoUrl } from './data'
...
export function FeaturesTabsSection() {
  ...
  let activeApp = apps[selectedIndex]!
  ...
  return (
    <section id="apps" className="border-y border-border/70 bg-surface py-20 sm:py-28">
      <Container wide>
        <FadeIn>
          <div className="mx-auto max-w-3xl text-center">
            <BracketLabel>Intuitive Experience, Powerful Features</BracketLabel>
            <SectionHeading className="mt-4">
              Features designed for{' '}
              <span className="text-brand">how you work</span>
            </SectionHeading>
          </div>
        </FadeIn>
            ...
            <TabList className="mx-auto flex max-w-5xl gap-1 overflow-x-auto rounded-2xl bg-surface-muted p-1.5 [-ms-overflow-style:none] [scrollbar-width:none] sm:rounded-full sm:p-2 [&::-webkit-scrollbar]:hidden">
              {apps.map((app, index) => (
                <Tab
                  ...
                  className={({ selected }) =>
                    clsx(
                      'relative shrink-0 cursor-pointer rounded-xl px-3 py-2.5 text-[13px] font-semibold whitespace-nowrap transition-colors duration-300 focus:outline-none sm:rounded-full sm:px-4 sm:py-2 sm:text-[14px]',
                ...
                >
                  {selectedIndex === index && (
                    <motion.span
                      ...
                      className="absolute inset-0 rounded-xl bg-brand shadow-brand sm:rounded-full"
              ...
              <AnimatePresence mode="wait" custom={direction} initial={false}>
                ...
                >
                  <div className="grid lg:grid-cols-2">
                    <motion.div
                      key={`panel-${activeApp.id}`}
                      initial={{ opacity: 0 }}
                      animate={{ opacity: 1 }}
                      transition={{ duration: 0.45, ease: easeOut }}
                      className="relative order-2 flex min-h-[280px] items-center justify-center overflow-hidden px-6 py-10 sm:min-h-[360px] sm:px-10 sm:py-12 lg:order-1 lg:min-h-[420px] lg:px-8 lg:py-10"
                      style={{ background: activeApp.panelBackground }}
                    >
                      <motion.div
                      ...
                      >
                        <BrowserMock
                          src={activeApp.image}
                          alt={`${activeApp.name} application in Arivu`}
                        />
                      </motion.div>
                    </motion.div>
                    ...
                    <motion.div
                      ...
                      initial="hidden"
                      animate="visible"
                      className="order-1 flex flex-col justify-center px-6 py-10 sm:px-10 sm:py-12 lg:order-2 lg:px-12 lg:py-14"
                    >
                      <motion.span
